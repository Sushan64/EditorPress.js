...
const ResizableImageTemplate = ({ node, updateAttributes }: NodeViewProps) => {
  ...
  const handleTouchStart = useEvent((event: React.TouchEvent<HTMLDivElement>) => {
    if (!imgRef.current) return;
    const direction = event.currentTarget.dataset.direction || "--";
    ...
    const touchMoveHandler = (event: TouchEvent) => {
      newWidth = Math.max(currentWidth + (transform * (event.touches[0].clientX - initialXPosition)), MIN_WIDTH);
      setResizingStyle({ width: newWidth });
    };
